...
- `Morse`: A family of exactly analytic wavelets. 
    `Morse(ga::T,be::T,cf::T) where T <: Float64`. `ga` characterizes the symmetry of the 
    Morse wavelet, `be` can be viewed as the decay or compactness parameter, and `cf` is a 
    parameter that determines the frequency at which the magnitude of the wavelet is maximized. 
